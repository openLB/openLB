  ...
  if ( util::intersect ( x0, x1, y0, y1,
    ...
    for (int iX=newX0; iX<=newX1; ++iX) {
      for (int iY=newY0; iY<=newY1; ++iY) {
        ...
        T blockOmega   = *(blockCell.template getFieldPointer<descriptors::OMEGA>()); //blockCell.getDynamics()->getOmega();
        T partnerOmega = *(partnerCell.template getFieldPointer<descriptors::OMEGA>()); //partnerCell.getDynamics()->getOmega();
